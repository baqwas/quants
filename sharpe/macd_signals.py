...
def plot_macd(data, ticker, fast_period=12, slow_period=26, signal_period=9):
    ...
    # Identify bullish (buy) and bearish (sell) crossover points
    # Bullish crossover: MACD crosses ABOVE Signal Line
    bullish_crossovers = (data_with_macd['MACD_Line'].shift(1) <= data_with_macd['Signal_Line'].shift(1)) & \
                         (data_with_macd['MACD_Line'] > data_with_macd['Signal_Line'])

    # Bearish crossover: MACD crosses BELOW Signal Line
    bearish_crossovers = (data_with_macd['MACD_Line'].shift(1) >= data_with_macd['Signal_Line'].shift(1)) & \
                         (data_with_macd['MACD_Line'] < data_with_macd['Signal_Line'])

    # Plotting crossover signals
    ax.scatter(data_with_macd.index[bullish_crossovers], data_with_macd['MACD_Line'][bullish_crossovers],
               marker='^', color='green', s=100, label='Buy Signal (Bullish Crossover)', zorder=5)
    ax.scatter(data_with_macd.index[bearish_crossovers], data_with_macd['MACD_Line'][bearish_crossovers],
               marker='v', color='red', s=100, label='Sell Signal (Bearish Crossover)', zorder=5)
